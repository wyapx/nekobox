...
@overload
def set_cfg(cfg: ConfigParser, section: str, option: str, description: str, *, unique: Optional[List[str]] = None) -> str: ...


@overload
def set_cfg(cfg: ConfigParser, section: str, option: str, description: str, default: str, unique: Optional[List[str]] = None) -> str: ...


def set_cfg(cfg: ConfigParser, section: str, option: str, description: str, default: Optional[str] = None, unique: Optional[List[str]] = None):
    if not unique:
        unique = []
    try:
        i = str(input(f"{description}{f' {cyan}({default}){reset}' if default else f' {cyan}(必填项){reset}'}: "))
        if not i:
            if not default:
                raise ValueError
            else:
...
def generate_cfg(args):
    ...
    set_cfg(cfg, uin, "log_level", "默认日志等级", default="INFO", unique=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"])
...
def _clear(args):
    if (bots := Path("./bots")).exists():
        if not args.uin:
            res = input(f"{gold}即将清理: {green}{bots.resolve()} {gold}下的所有数据，是否继续? {bd}{magnet}[y/n] {cyan}(y): {reset}")
            if res.lower() == "y":
                shutil.rmtree(bots)
                print(f"{green}{bots.resolve()}{reset} 清理完毕")
        elif (dir_ := (bots / str(args.uin))).exists():
            res = input(f"{gold}即将清理: {green}{dir_.resolve()} {gold}下的所有数据，是否继续? {bd}{magnet}[y/n] {cyan}(y): {reset}")
            if res.lower() == "y":
                shutil.rmtree(dir_)
